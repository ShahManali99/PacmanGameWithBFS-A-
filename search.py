...
def breadthFirstSearch(problem):
    ...
    queue = util.Queue()
    queue.push(Node(root,None,None,0))
    while(not queue.isEmpty()):
        node = queue.pop()
        visited.add(node.state)
        if problem.goalTest(node.state): return getFinalPath(problem,node)
        list = problem.getActions(node.state)
        for action in list:
            nextState = problem.getResult(node.state,action)
            if nextState not in visited:
                queue.push(Node(nextState,node,action,0))
    # util.raiseNotDefined()

def getFinalPath(problem,goalNode):
    path = []
    while(goalNode.state!=problem.getStartState()):
        path.append(goalNode.action)
...
def aStarSearch(problem, heuristic=nullHeuristic):
    ...
    pq.push(Node(root,None,None,0),0+heuristic(root,problem))
    while(not pq.isEmpty()):
        ...
        for action in list:
            nextState = problem.getResult(node.state,action)
            if nextState not in visited:
                g = problem.getCost(node.state,action)+node.path_cost
                pq.push(Node(nextState,node,action,g),g+heuristic(nextState,problem))
    #util.raiseNotDefined()
